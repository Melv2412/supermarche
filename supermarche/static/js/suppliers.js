// Création de l'objet global App s'il n'existe pas
window.App = window.App || {};

// Module Suppliers
window.App.Suppliers = (function() {
  ...
  function gotoSupplierDetail(id) { 
    window.location.href = `/suppliers/${id || 1}/`; 
  }
  
  function gotoSupplierForm() { 
  ...
  function gotoOrderDetail(id) { 
    window.location.href = `/suppliers/orders/detail/?id=${id || 1}`; 
  }
  
  function gotoOrderForm() { 
    window.location.href = '/suppliers/orders/create/'; 
  }
    ...
    try {
      ...
      function render() {
        ...
        filtered.forEach(s => {
          ...
          tr.innerHTML = `
            <td>
              <div class="font-medium text-violet-600">${s.name}</div>
            ...
            <td>
              <div class="flex flex-wrap gap-1">
                ${(s.categories || []).map(cat => 
                  `<span class="badge badge-violet">${cat}</span>`
                ).join('') || '-'}
              </div>
            </td>
            <td class="text-center">
              <span class="font-bold">${s.order_count || 0}</span>
            </td>
            <td class="text-center">
            ...
            <td class="text-center">
              <button onclick="App.Suppliers.gotoSupplierDetail(${s.id})" 
      ...
      alert('Impossible de charger la liste des fournisseurs');
    }
  }
  ...
  async function initSupplierDetail() {
    try {
      ...
      const id = suppliersIndex >= 0 ? Number(pathParts[suppliersIndex + 1]) : 1;
      
      console.log('Loading supplier detail for ID:', id);
      
      const [suppliers, orders] = await Promise.all([loadSuppliers(), loadOrders()]);
      const sup = suppliers.find(s => s.id === id);
      
      if (!sup) {
        console.error('Fournisseur non trouvé:', id);
        document.getElementById('sup-name').textContent = 'Fournisseur introuvable';
        return;
      }
      
      document.getElementById('sup-name').textContent = sup.name || 'Sans nom';
      ...
      if (related.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-gray-500 py-4">Aucune commande</td></tr>';
      } else {
        related.forEach(o => {
          const tr = document.createElement('tr');
          tr.innerHTML = `
            ...
            <td class="text-end">${fmtPrice(o.amount)}</td>
          `;
          tbody.appendChild(tr);
        });
      }
    } catch(e) { 
      console.error('Erreur détail fournisseur:', e);
      alert('Erreur lors du chargement des détails du fournisseur');
    }
  }
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        const formData = {
          ...
          adresse: address.value.trim() || null,
          ville: 'NON PRECISE', // null pour champs optionnels
          delai_livraison_jours: 0,
          montant_commande_min: 0,
          conditions_paiement: 'RIEN', // null pour champs optionnels
        ...
        if (response.ok) {
          ...
          alert('Erreur : ' + (error.nom?.[0] || error.email?.[0] || JSON.stringify(error)));
        }
      } catch (error) {
  ...
  async function initOrderList() {
    try {
      ...
      console.error('Erreur liste commandes:', e); 
    }
  }
    ...
    try {
      ...
      o.lines.forEach(l => {
        ...
        tr.innerHTML = `
          ...
          <td class="text-end">${fmtPrice(l.unit_price)}</td>
          <td class="text-end">${fmtPrice(lineTotal)}</td>
        `;
        tbody.appendChild(tr);
      });
      ...
      console.error('Erreur détail commande:', e); 
    }
  }
  ...
  async function initOrderForm() {
    ...
    function recalc() {
      ...
      tbody.querySelectorAll('tr').forEach(tr => { 
        ...
        const pu = Number(tr.querySelector('[data-unit]').value || 0); 
        total += q * pu; 
      });
    ...
    function addLine() {
      ...
      tr.innerHTML = `
        ...
        <td class="text-end" data-line>0 FCFA</td>
      ...
      function update() { 
        const v = Number(qty.value || 0) * Number(unit.value || 0); 
        line.textContent = fmtPrice(v); 
        recalc(); 
      }
...
window.Suppliers = window.App.Suppliers;